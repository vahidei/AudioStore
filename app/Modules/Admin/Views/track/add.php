    ...
    <div class="row mt-3">
        ...
        <div class="col-md-6 mt-3 mt-md-0">
            <div class="row">
                <div class="col-sm-6">
                    <h6>Price (Rials):</h6>
                    <input type="number" name="price" class="form-control" min="0" value="<?= $post['price']; ?>"/>
                </div>
                <div class="col-sm-6 mt-3 mt-sm-0">
                    <h6 class="cursor-pointer">
                        <input class="cursor-pointer" <?= (isset($post['buy_limit']) ? 'checked' : ''); ?>
                               type="checkbox" id="buylimitdisabled">
                        <label class="cursor-pointer" for="buylimitdisabled">Buy limit:</label>
                    </h6>
                    <input type="number" id="buy_limit" name="buy_limit"
                           class="form-control" <?= (!isset($post['buy_limit']) ? 'disabled' : ''); ?> min="0"
                           value="<?= $post['buy_limit']; ?>"/>
                </div>
            </div>
        </div>
    </div>
    <div class="row mt-3">
        <div class="col-md-6">
            ...
            <select name="category" class="form-control">
                <option disabled <?= (empty($post['category'])) ? 'seleceted' : ''; ?>>Select a category--</option>
                <?php
                foreach ($data['category_list'] as $key => $value) {
                    echo '<option ' . (($post['category'] == $value['id']) ? 'seleceted' : '') . ' value="' . $value['id'] . '" >' . $value['title'] . '</option>';
                }
                ?>
            </select>
        </div>
        <div class="col-md-6">
            <div class="row statusBox">
                <div class="col-lg-<?= ($post['status'] == 'scheduling') ? '6' : '12'; ?> selectCol">
                    <h6>Status:</h6>
                    <select name="status" class="form-control">
                        <option value="publish" <?= ($post['status'] == 'publish') ? 'selected' : ''; ?>>Publish
                        </option>
                        <option value="draft" <?= ($post['status'] == 'draft') ? 'selected' : ''; ?>>Draft</option>
                        <option value="scheduling" <?= ($post['status'] == 'scheduling') ? 'selected' : ''; ?>>
                            Scheduling
                        </option>
                        <option value="only_package" <?= ($post['status'] == 'only_package') ? 'selected' : ''; ?>>Only
                            use in packages
                        </option>
                    </select>
                </div>
                <div class="col-lg-6 schedulingCol <?= ($post['status'] == 'scheduling') ? '' : 'd-none'; ?>">
                    <h6>Choose a day:</h6>
                    <input type="datetime-local" class="form-control" name="action_time" value="<?= $post['action_time']; ?>">
                    <div class="mt-1">
                        <label>
                            <input type="radio" name="action_type" <?= ($post['action_type'] == 'publish') ? 'checked' : ''; ?> value="publish"/> Publish on that day
                        </label>
                    </div>
                    <label>
                        <input type="radio" name="action_type" <?= ($post['action_type'] == 'draft') ? 'checked' : ''; ?> value="draft"/> Draft on that day
                    </label>
                </div>
            </div>
        </div>
