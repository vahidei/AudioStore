...
<form action="" method="POST" enctype="multipart/form-data">
        ...
        <div class="col-12">
            ...
            <select name="tracks[]" data-live-search="true" id="track_select" class="selectpicker form-control" multiple>
                ...
                foreach($data['tracks'] as $key => $item){
                    $selected = (is_array($post['tracks']) && in_array($item['id'], $post['tracks'])) ? 'selected' : '';
    ...
    <div class="row mt-3">
        ...
        <div class="col-md-6 mt-3 mt-md-0">
            ...
            <textarea name="short_desc_fa" class="rtl form-control" rows="5"><?=$post['short_desc_fa'];?></textarea>
        </div>
    </div>
    <div class="row mt-3">
        <div class="col-md-6">
            <h6>More description:</h6>
            <textarea name="more_desc" class="form-control" rows="5"><?=$post['more_desc'];?></textarea>
        </div>
        <div class="col-md-6 mt-3 mt-md-0">
            <h6>Persian more description:</h6>
            <textarea name="more_desc_fa" class="rtl form-control" rows="5"><?=$post['more_desc_fa'];?></textarea>
        </div>
    </div>
    <div class="row mt-3">
        <div class="col-md-6">
            ...
            <div class="row">
                ...
                <div class="col-sm-6 mt-3 mt-sm-0">
                    <h6 class="cursor-pointer">
                        <input class="cursor-pointer" <?=(isset($post['buy_limit']) ? 'checked' : '');?> type="checkbox" id="buylimitdisabled">
                        <label class="cursor-pointer" for="buylimitdisabled">Buy limit:</label>
                    </h6>
                    <input type="number" id="buy_limit" name="buy_limit" class="form-control" <?=(!isset($post['buy_limit']) ? 'disabled' : '');?> min="0" value="<?=$post['buy_limit'];?>"/>
                </div>
            </div>
        </div>
    </div>
    <div class="mt-3 mb-5">
        <h6>
            <input type="checkbox" <?=($post['saveindrafts'] == 'on') ? 'checked' : '';?> name="saveindrafts" id="saveindrafts">
